...
Currently contains vector and list. The code is generic as it uses a text replacement tool `def.py` or `def` generated by `make bootstrap`. These programs are configured by a JSON file as follows:
```json
{
    "vectordef.h": [
        {
